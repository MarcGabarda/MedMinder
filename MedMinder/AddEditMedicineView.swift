...
struct AddEditMedicineView: View {
    ...
    // Dark, rich colours — always readable on white/light backgrounds
    let colorOptions: [Color] = [
        Color(hex: "#DC2626")!, // deep red
        Color(hex: "#EA580C")!, // deep orange
        Color(hex: "#D97706")!, // amber
        Color(hex: "#059669")!, // emerald
        Color(hex: "#0284C7")!, // sky blue
        Color(hex: "#2563EB")!, // royal blue
        Color(hex: "#7C3AED")!, // violet
        Color(hex: "#BE185D")!, // hot pink
    ...
    private func loadExistingValues() {
        ...
        name = m.name
        dosage = m.dosage.truncatingRemainder(dividingBy: 1) == 0 ? String(Int(m.dosage)) : String(m.dosage)
    ...
    private func save() {
        ...
        switch mode {
        case .add:
            store.add(Medicine(name: name.trimmingCharacters(in: .whitespaces), dosage: val, unit: unit.rawValue, colorHex: selectedColor.toHex(), notes: notes, reminderTime: reminderTime, reminderDays: Array(selectedDays)))
        case .edit(let m):
            m.name = name.trimmingCharacters(in: .whitespaces)
            m.dosage = val; m.unit = unit.rawValue
            m.colorHex = selectedColor.toHex()
            m.notes = notes; m.reminderTime = reminderTime
